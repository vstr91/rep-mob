...
public class MusicaDBAdapter {
    ...
    public List<Musica> listarRelacionadasPorTom(Musica musica){
        Cursor cursor = database.rawQuery("SELECT _id, nome, tom, id_artista, status, data_cadastro, data_recebimento, " +
                "ultima_alteracao, slug, id_estilo FROM musica WHERE tom = ? AND _id != ? ORDER BY nome COLLATE LOCALIZED", new String[]{musica.getTom(), musica.getId()});
        List<Musica> musicas = new ArrayList<Musica>();
